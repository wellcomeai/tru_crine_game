...
import { MapPin, Users, Search, ClipboardList, BookOpen, Scale } from 'lucide-react';
...
export default function Sidebar() {
  ...
  return (
    <div className="w-16 lg:w-48 bg-noir-800 border-r border-noir-600 flex flex-col py-4">
      {menuItems.map((item) => {
        ...
        return (
          ...
          >
            ...
            {item.id === 'evidence' && evidenceCount > 0 && (
              <span className="hidden lg:block ml-auto text-xs bg-gold text-noir-900 rounded-full px-1.5 py-0.5">
                {evidenceCount}
              </span>
            )}
          </button>
